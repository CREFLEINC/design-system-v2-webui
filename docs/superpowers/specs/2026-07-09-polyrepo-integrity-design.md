...
이 폰트 6개는 repo 안에 **두 벌**이 바이트 동일하게 존재한다 — `styles/foundation/fonts/`(파운데이션 동봉본)와 `ds-bundle/fonts/`(Claude Design sync가 쓰는 자체 완결형 번들, `ds-bundle/fonts.css`가 상대경로로 물고 있다). SIL OFL 1.1과 Apache-2.0 모두 **배포되는 사본마다** 라이선스 전문 동봉을 요구하므로, 라이선스 파일도 두 위치 모두에 있어야 한다. 한쪽만 챙기고 다른 쪽을 빠뜨리기 쉬운 지점이다.

| 폰트 | 라이선스 | 추가할 파일 |
|---|---|---|
| Spoqa Han Sans Neo (Thin/Light/Regular/Medium/Bold) | SIL OFL 1.1 | `styles/foundation/fonts/LICENSE-SpoqaHanSansNeo.txt`<br>`ds-bundle/fonts/LICENSE-SpoqaHanSansNeo.txt` |
| Material Symbols Rounded | Apache-2.0 | `styles/foundation/fonts/LICENSE-MaterialSymbols.txt`<br>`ds-bundle/fonts/LICENSE-MaterialSymbols.txt` |

`styles/foundation/fonts/LICENSE-*.txt` 두 파일은 파운데이션에서 동기화된 것이 아니므로 lock의 `files`에 등재되지 않는다. 그대로 두면 `verifyMirror`의 4번 검사(미등록 파일 = 에러)에 걸린다. 그래서 §1의 허용 목록에 `fonts/LICENSE-*.txt`가 처음부터 포함돼 있다 — 2단계에서 검사기를 구현할 때 이미 반영하고, 5단계는 파일만 추가한다. 반면 `ds-bundle/fonts/`는 `verifyMirror`의 검사 범위 밖(오직 `styles/foundation/`만 본다)이라 애초에 lock·허용 목록과 무관하다 — 이 위치는 순전히 사람이 챙겨야 한다.

public 전환 전에는 `git ls-files | grep -Ei '\.(woff2?|ttf|otf)$'`로 repo 안의 폰트 사본을 전부 찾아 **경로마다** 라이선스 파일이 동봉됐는지 확인한다. 새 사본이 추가되면(예: 다른 번들, 다른 도메인 DS) 이 grep을 다시 돌려야 빠뜨린 사본이 없다.
...
`enforce_admins`는 실측을 거쳐 **`true`**로 정했다. `false`로 걸어 보면 `main` 직접 push 시 GitHub이 `Required status check "check" is expected.`를 **경고**로만 찍고 org admin의 push를 그대로 받아들인다 — repo 소유자에게는 보호가 사실상 장식이 된다. `true`로 걸면 같은 push가 `! [remote rejected] main -> main (protected branch hook declined)`로 거부된다. 둘 다 이 프로젝트에서 직접 관찰했다. 긴급 우회가 필요하면 GitHub 설정 화면에서 `enforce_admins`를 그때 토글하면 된다 — API 값 자체를 평소에 느슨하게 둘 이유는 없다.
...
- **nightly upstream cron** — 파운데이션이 사실상 정지 상태(마지막 토큰 변경 이후 커밋 9개가 전부 문서 변경, `tokens.css`를 건드린 커밋 0개)라 과하다. **[2026-07 갱신: 파운데이션 재활성화(이슈 #2 오픈)로 전제가 깨져, #4 에서 주간 `upstream.yml` + `--strict` 모드를 추가했다.]**
