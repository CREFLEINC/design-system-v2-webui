...
- Create: `ds-bundle/fonts/LICENSE-SpoqaHanSansNeo.txt`
- Create: `ds-bundle/fonts/LICENSE-MaterialSymbols.txt`
- Modify: `package.json` (`license` 필드)
- Modify: `ds-bundle/README.md` (서드파티 폰트 고지)

**주의: 폰트 사본이 repo 안에 두 벌이다.** `styles/foundation/fonts/`(파운데이션 동봉본)와
`ds-bundle/fonts/`(Claude Design sync가 쓰는 자체 완결형 번들, `ds-bundle/fonts.css`가 상대경로로
물고 있다)에 같은 폰트 6개가 바이트 동일하게 들어 있다. SIL OFL 1.1·Apache-2.0 모두 **배포되는
사본마다** 라이선스 전문 동봉을 요구하므로 두 위치 다 챙긴다. `ds-bundle/fonts/`는
`verifyMirror`의 검사 범위 밖(오직 `styles/foundation/`만 본다)이라 빠뜨려도 `npm run check`가
잡아주지 않는다 — 사람이 직접 확인해야 한다.

**Interfaces:**
- Consumes: Task 1의 허용 목록 정규식 `^fonts/LICENSE-[^/]+\.txt$` — `styles/foundation/fonts/` 쪽 두 파일이 미등록 파일로 잡히지 않는 이유. `ds-bundle/fonts/`는 이 허용 목록과 무관하다(애초에 lock 검사 대상이 아니므로).

- [ ] **Step 1: 폰트 라이선스 전문을 받아 두 위치에 둔다**
...
cp styles/foundation/fonts/LICENSE-SpoqaHanSansNeo.txt ds-bundle/fonts/LICENSE-SpoqaHanSansNeo.txt
cp styles/foundation/fonts/LICENSE-MaterialSymbols.txt ds-bundle/fonts/LICENSE-MaterialSymbols.txt
```
...
- [ ] **Step 3: 미등록 파일로 잡히지 않는지, 그리고 폰트 사본을 전부 찾았는지 확인한다**

Run: `node scripts/check-foundation.mjs`
Expected: `✓ 파운데이션 미러 무결성 OK — 7개 파일 @ …` / exit 0
(`styles/foundation/fonts/` 의 라이선스 2개는 허용 목록에 걸려 files 카운트에 들어가지 않는다.
`ds-bundle/fonts/`는 이 검사 범위 밖이므로 여기서는 아무 신호도 주지 않는다.)

Run: `git ls-files | grep -Ei '\.(woff2?|ttf|otf)$'`
Expected: `styles/foundation/fonts/*.woff2` 6개와 `ds-bundle/fonts/*.woff2` 6개, 총 12줄.
이 명령으로 나온 경로마다 같은 디렉토리에 `LICENSE-*.txt`가 있는지 확인한다 — 새 번들이
생기면 이 grep을 다시 돌려 빠뜨린 사본이 없는지 재확인한다.
...
This repository bundles font files that are licensed separately. The same
fonts are distributed twice — once under styles/foundation/fonts/ (source)
and once under ds-bundle/fonts/ (self-contained sync bundle) — and each
copy carries its own license texts alongside it:

  Spoqa Han Sans Neo (Thin/Light/Regular/Medium/Bold)
    SIL Open Font License 1.1
    styles/foundation/fonts/LICENSE-SpoqaHanSansNeo.txt
    ds-bundle/fonts/LICENSE-SpoqaHanSansNeo.txt

  Material Symbols Rounded
    Apache License 2.0
    styles/foundation/fonts/LICENSE-MaterialSymbols.txt
    ds-bundle/fonts/LICENSE-MaterialSymbols.txt
...
- [ ] **Step 5: `package.json` 에 license 필드를 추가하고, `ds-bundle/README.md` 에 서드파티 고지를 더한다**
...
`ds-bundle/README.md`에는 "서드파티 폰트 라이선스" 절을 추가해 `fonts/*.woff2` 6개가 서드파티
자산이며 각 라이선스와 `fonts/LICENSE-*.txt` 경로를, 그리고 같은 폰트가
`styles/foundation/fonts/`에도 존재해 두 위치 모두 라이선스 텍스트를 동봉한다는 점을 명시한다.
...
```bash
git add LICENSE package.json ds-bundle/README.md \
  styles/foundation/fonts/LICENSE-SpoqaHanSansNeo.txt styles/foundation/fonts/LICENSE-MaterialSymbols.txt \
  ds-bundle/fonts/LICENSE-SpoqaHanSansNeo.txt ds-bundle/fonts/LICENSE-MaterialSymbols.txt
git commit -F - <<'EOF'
...
같은 폰트가 styles/foundation/fonts/ 와 ds-bundle/fonts/ 두 곳에 사본으로 존재하므로
라이선스 파일도 두 곳 모두에 둔다. ds-bundle/fonts/ 는 verifyMirror 의 검사 범위 밖이라
lock 이 지켜주지 않는다 — LICENSE 와 ds-bundle/README.md 에 이 사실을 적는다.
...
`enforce_admins`는 실측을 거쳐 **`true`**로 건다. `false`로 걸어 본 결과, `main` 직접 push 시
GitHub은 `Required status check "check" is expected.`를 **경고**로만 찍고 org admin의 push를
그대로 받아들였다 — repo 소유자에게는 보호가 사실상 장식이 되는 것이다. `true`로 걸면 같은 push가
Step 3에서 보듯 `protected branch hook declined`로 실제 거부된다. 관리자도 우회 불가능하게 두는
편이 1인 개발에서는 더 안전하다. 긴급 우회가 필요하면 그때 GitHub 설정 화면에서
`enforce_admins`를 토글한다 — API 기본값을 평소에 느슨하게 열어 둘 이유는 없다.
...
```
Expected: `{"checks":["check"],"pr_required":true,"admins":true}`
...
| `styles/foundation/fonts/LICENSE-*.txt` | 그대로 | 폰트를 번들한다면 **필수** — 사본이 여러 경로에 있다면 전부에 (아래 "repo 설정" 참고) |
...
| `enforce_admins` | **true** | false면 org admin에게는 protection이 장식일 뿐 — 아래 참고 |

**`enforce_admins`는 반드시 `true`로 설정한다.** `false`로 두면 GitHub은 org admin의
`main` 직접 push를 실제로는 막지 않는다. 우리는 이를 직접 확인했다: `enforce_admins: false`
상태에서 admin 계정으로 push하면 `Required status check "check" is expected.`를
**경고**로만 찍고 push를 그대로 받아준다. 같은 설정을 `enforce_admins: true`로 바꾸고
동일한 push를 재시도하면 `! [remote rejected] (protected branch hook declined)`로
실제 거부된다. 즉 `enforce_admins`를 켜지 않으면, protection은 org를 소유한 사람에게는
아무 효력이 없는 장식이다.

작동을 확인한 실제 호출:

```bash
gh api -X PUT repos/CREFLEINC/<repo>/branches/main/protection \
  -H "Accept: application/vnd.github+json" --input - <<'JSON'
{
  "required_status_checks": { "strict": true, "contexts": ["check"] },
  "enforce_admins": true,
...
전문 포함을 요구한다 — 그것도 **배포되는 사본 전부**에 대해서다. 웹 DS에는 같은 여섯
폰트 파일이 두 곳에 존재한다: `styles/foundation/fonts/`(파운데이션 미러)와
`ds-bundle/fonts/`(Claude Design sync가 쓰는 독립 번들, `ds-bundle/fonts.css`가 상대경로로
참조). 처음엔 첫 번째 경로에만 라이선스 전문을 넣었는데, 그대로 뒀다면 두 번째 경로는
라이선스 없이 public이 될 뻔했다. **폰트를 두 곳에 복사한다면 두 곳 모두에 라이선스
전문을 둔다.** publish 전에 다음으로 사본을 전부 찾는다:

```bash
git ls-files | grep -Ei '\.(woff2?|ttf|otf)$'
```

나온 경로마다 같은 디렉토리에 `LICENSE-*.txt`가 있는지 확인한다. 하나라도 없으면
그 사본은 무단 재배포다.
...
- [ ] `LICENSE` + 번들 폰트 라이선스 전문 — `git ls-files | grep -Ei '\.(woff2?|ttf|otf)$'`로
      폰트 사본을 전부 찾아 **경로마다** 확인
- [ ] `.github/workflows/ci.yml` (잡 이름 `check`)
- [ ] 첫 CI run이 green인지 확인
- [ ] branch protection (`contexts: ["check"]`, **`enforce_admins: true`**)
