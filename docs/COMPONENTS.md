...
`@crefle/web-ui` v0.2.0 — 모듈 39개에서 컴포넌트 46개 · 훅 3 · 유틸 1
...
`src/components/TextArea/TextArea.tsx` · 스토리 ✓

**TextArea**
- props `TextAreaProps`: label, helperText, error, disabledReason, size, fullWidth, containerClassName, rows, maxRows, resize, maxLength, showCount _(+ 네이티브 HTML 속성)_
...
`src/components/NumberPad/NumberPad.tsx` · 스토리 ✓
...
**Table**
- props `TableProps`: columns, rows, getRowId, caption, density, zebra, sort, defaultSort, onSortChange, selectable, selectedIds, onSelectionChange, empty, summaryRows, groupBy, renderGroupHeader, reorderable, onRowReorder _(+ 네이티브 HTML 속성)_
...
`src/components/DiffRow/DiffRow.tsx` · 스토리 ✓

**DiffRow**
- props `DiffRowProps`: label, before, after, emptyText, changed _(+ 네이티브 HTML 속성)_
...
`src/components/MatrixGrid/MatrixGrid.tsx` · 스토리 ✓

**MatrixGrid**
- props `MatrixGridProps`: columns, rows, groupHeaders, summaryRow, renderCell, onCellClick, size _(+ 네이티브 HTML 속성)_
