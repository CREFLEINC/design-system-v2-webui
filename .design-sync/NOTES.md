...
- This is the WEB LAYER of the CREFLE DS (46 components, @crefle/web-ui). Separate from CREFLE Foundation (`7ab62ce9-...`, foundation only) and CREFLE Presentation Design System (slides).
...
- Re-sync: rebuild ds-bundle (copy latest tokens/themes/fonts, regenerate cards), `: > ds-bundle/_ds_needs_recompile`, then DesignSync finalize_plan(writes = ds-bundle paths, localDir = ds-bundle) -> write_files. No deletes unless removing a card.
  - `guidelines/component-conventions.md` · `patterns.md` · `reduced-motion.md` are **copies of `docs/`** (authored there; the bundle ships them because it leaves the repo and cannot reference `docs/` by relative path). Refresh them with `npm run sync:bundle-guidelines` — never edit the bundle side. `npm run check:bundle-guidelines` (wired into `npm run check`) fails the build when the two sides drift. `guidelines/colors.md` · `tokens.md` · `type.md` have no `docs/` counterpart and are bundle-native.
