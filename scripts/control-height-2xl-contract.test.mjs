// #90: TextField·SearchInput·Select·NumberPad 4종의 2xl(72px) 등급이 공용 토큰
// --control-height-2xl을 소비함을 고정한다.
// jsdom은 CSS 미주입(vitest css 기본 false)이라 CSS 모듈은 키를 그대로 돌려주는
// 프록시다 — styles.xxl은 CSS에 .xxl 규칙이 없어도 'xxl'을 반환하므로 클래스명
// 단언만으로는 높이 바인딩이 증명되지 않는다. 그래서 node:fs로 소스를 직접 읽어
...
test('회귀: 4개 모듈 CSS의 .xl 블록은 여전히 --control-height-xl을 참조한다(순수 가산)', () => {
