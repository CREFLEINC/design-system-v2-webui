 ...
 * 팝업이 현재 배치를 data-placement로 노출해 단언 대상이 된다.
...
function Harness() {
  ...
  return createElement(
    ...
    open &&
      createElement(
        'div',
        { ref: popupRef, 'data-testid': 'popup', 'data-placement': placement },
        '팝업',
      ),
  )
}
...
test('S1 — 아래 공간이 충분하면 down (flip 없음)', async () => {
  ...
  expect(screen.getByTestId('popup')).toHaveAttribute('data-placement', 'down')
})
...
test('S2 — 아래가 부족하고 위가 충분하면 up으로 뒤집는다', async () => {
  ...
  expect(screen.getByTestId('popup')).toHaveAttribute('data-placement', 'up')
})
...
test('리셋 시맨틱스 — 재오픈 시 언제나 "아래 배치 상태"를 실측한다', async () => {
  const user = userEvent.setup()
  setViewportHeight(800)
  // 실제 레이아웃을 흉내 내는 모킹: 팝업 rect가 현재 배치(data-placement)에 따라 달라진다.
  // 트리거 top 600 / bottom 640, 팝업 높이 300, 간격 4.
  vi.spyOn(Element.prototype, 'getBoundingClientRect').mockImplementation(function (this: Element) {
    const id = this.getAttribute('data-testid')
    if (id === 'trigger') return makeRect({ top: 600, bottom: 640 })
    if (id === 'popup')
      return this.getAttribute('data-placement') === 'up'
...
test('제로-rect(모킹 없는 jsdom 기본)에서는 flip이 일어나지 않는다', async () => {
  ...
  // 모든 rect가 0 → overflowsBelow = 0 > 768 = false → 기존 테스트에 영향 없음
  expect(screen.getByTestId('popup')).toHaveAttribute('data-placement', 'down')
})
