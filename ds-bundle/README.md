...
그대로 상속하면서, 그 위에 **형태 토큰(type·space·radius·elevation·motion) + 46개 React 컴포넌트**를
...
| **Tier 1 — 폼·기본 (Phase 2)** | IconButton, TextField, Select, Checkbox, Radio, Switch, Chip, Badge, Card, Dialog, Tooltip, Tabs, DatePicker, NumberPad, TextArea |
| **Tier 2 — 데이터·피드백 (Phase 3)** | Table, StatCard, Progress, Gauge, AlertBanner, Toast, Navigation(Sidebar/Topbar), Breadcrumb, EmptyState, Skeleton, SearchInput, Stepper, DiffRow |
...
위 표는 주요 컴포넌트만 나열하며, 하위 컴포넌트·훅까지 포함한 전체 46개 실측 목록은 `docs/COMPONENTS.md`에 있다.)

최근 확장: **TextArea**(여러 줄 텍스트 입력 — TextField 계약 공유(label·helperText·error·disabledReason·size·fullWidth), rows→maxRows 자동 높이, soft maxLength 글자 수 카운터), **DatePicker**(단일·기간(from–to) 날짜 입력 — 자체 달력 렌더, from≤to 보장, xl 터치 정합),
**DiffRow**(변경 이력의 값 대비 — before→after 잎 컴포넌트, 빈값/삭제/미변경 표기, `--semantic-idle-text`),
